...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const lengthBetween = x1 * x2 + y1 * y2;
  const lengthFirst = Math.sqrt(x1 ** 2 + y1 ** 2);
  const lengthSecond = Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(lengthBetween / (lengthFirst * lengthSecond));
}
